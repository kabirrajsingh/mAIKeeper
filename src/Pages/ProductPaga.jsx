...
const ProductPaga = () => {
  ...
  useEffect(() => {
    ...
    let productItem = item.filter(item => item.id == id);
    ...
    <div style={{ backgroundColor: "var(--white-color)" }}>
      <main id="container">
        ...
        <div className="ProductPaga-container top-bottom-padding left-right-padding">
          ...
          <div className="product-buying-detail">
...
export const EmiSection = () => {
  return (
    <div className="emi-section">
      <p className="bold">No Cost EMI</p>
      <p>Upto ₹85.51 EMI interest savings on aiShopper Pay ICICI…</p>
    ...
    <>
      {Product.map((item) => (
        <div className="shipping-Icon" key={item.id}>
          <img src={`../image/ProductImage/${item.image}.png`} alt={item.name} />
      ...
      <div className="flexBox top-bottom-padding" style={{ justifyContent: 'start' }}>
        {Product.map((item) => (
          <div className="color-selector" key={item.id}>
            <img onClick={() => changeColor(item)} src={`../image/ProductImage/${item.image}.jpg`} alt={item.name} />
            <p>₹ {item.price}</p>
          </div>
        ))}
      </div>
    ...
    <>
      {Product.map((item) => (
        <div className="small-product" key={item.id}>
          <img src={`../image/ProductImage/${item.image}.jpg`} alt={item.name} />
        </div>
      ))}
    </>
  );
